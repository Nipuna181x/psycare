...
<nav class="mx-auto flex max-w-[1320px] items-center justify-between gap-4 px-5 py-6 md:px-9 md:py-7">
    ...
    <div class="hidden items-center gap-1 rounded-full bg-card px-2 py-1.5 shadow-[0_1px_0_0_var(--border)] lg:flex">
        @foreach ([['Home', '/'], ['Book a Doctor', route('doctors.index')], ['Lumi', '/ai-companion'], ['My Health Records', route('health-records.index')], ['My Appointments', route('appointments.index')], ['Group Therapy', route('therapy-rooms.index')], ['Mood Tracker', '/mood-tracker']] as [$label, $href])
