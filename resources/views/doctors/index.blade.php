...
<body>
    ...
    <div class="min-h-screen bg-background text-ink selection:bg-teal/20">
        <nav class="mx-auto flex max-w-[1320px] items-center justify-between gap-4 px-5 py-6 md:px-9 md:py-7">
            ...
            <div class="hidden items-center gap-1 rounded-full bg-card px-2 py-1.5 shadow-[0_1px_0_0_var(--border)] lg:flex">@foreach ([['Home', '/'], ['Book a Doctor', '/doctors'], ['AI Companion', '/ai-companion'], ['My Health Records', '/health-records'], ['My Appointments', '/appointments'], ['Group Therapy', '/group-therapy'], ['Mood Tracker', '/mood-tracker']] as [$label, $href])<a href="{{ $href }}" class="whitespace-nowrap rounded-full px-3 py-2 text-[12px] text-ink-soft transition-colors hover:bg-secondary hover:text-ink {{ request()->is(ltrim($href, '/')) ? 'bg-secondary text-ink' : '' }}">{{ $label }}</a>@endforeach</div>
